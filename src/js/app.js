...
function renderizarCatalogo() {
  ...
  console.log(`✅ ${productos.length} productos renderizados en el catálogo`);
}
...
function crearTarjetaProducto(producto) {
  ...
  div.innerHTML = `
    ...
    ${producto.personalizable ? 
      '<p class="producto-precio-desde">Desde $' + formatearPrecio(precioMostrar) + '</p>' :
      '<p class="producto-precio">$' + formatearPrecio(precioMostrar) + '</p>'
    }
...
function inicializarEventListeners() {
  if (btnCarrito) {
    btnCarrito.addEventListener('click', toggleCarritoPanel);
  }
  
  if (btnCerrarCarrito) {
    btnCerrarCarrito.addEventListener('click', cerrarCarritoPanel);
  }
