...
function actualizarEstadisticas() {
  ...
  if (document.getElementById('stat-ventas-hoy')) {
    document.getElementById('stat-ventas-hoy').textContent = '$' + formatearPrecio(estadisticasActuales.ventasHoy || 0);
  }
  if (document.getElementById('stat-pedidos-hoy')) {
    document.getElementById('stat-pedidos-hoy').textContent = estadisticasActuales.pedidosHoy || 0;
  }
  if (document.getElementById('stat-promedio-venta')) {
    document.getElementById('stat-promedio-venta').textContent = '$' + formatearPrecio(estadisticasActuales.promedioVenta || 0);
  }
  if (document.getElementById('stat-clientes-unicos')) {
    document.getElementById('stat-clientes-unicos').textContent = estadisticasActuales.clientesUnicos || 0;
  }
}
...
async function cambiarEstadoPedido(pedidoId) {
  ...
  try {
    ...
    if (typeof ErrorHandler !== 'undefined') {
      ...
      toastManager.error(error.message || 'Error actualizando estado');
    } else {
...
function inicializarEventListeners() {
  ...
  if (sidebarToggle && sidebar) {
    sidebarToggle.addEventListener('click', () => {
      ...
      sidebarToggle.setAttribute('aria-label', isActive ? 'Cerrar menú' : 'Abrir menú');
  ...
  navLinks.forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      if (sectionEl) {
        ...
        // Inicializar módulo de productos si es necesario
        if (section === 'productos' && typeof adminProductos !== 'undefined') {
          if (!adminProductos.initialized) {
            adminProductos.init();
            adminProductos.initialized = true;
          }
        }
      }
    });
